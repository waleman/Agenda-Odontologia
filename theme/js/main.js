$(document).ready(function(){
	...
	$('.btn-dialog-config').on('click', function(){
		$('#Dialog-config').modal('show');
		return false;
	});

	$('.btn-exit-lang').on('click', function(){
		$('#Dialog-lenguaje').modal('show');
		return false;
	});
	$('#btnidioma').click(function(){
		let valor = $("#comboidioma").val();
		var d = new Date();
		d.setTime(d.getTime() + (2 * 24 * 60 * 60 * 1000));
		var expires = "expires="+d.toUTCString();
		document.cookie = "idioma=" + valor + ";" + expires + ";path=/";
		location.reload();
	});
